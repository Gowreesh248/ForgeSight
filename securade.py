...
def process():
    ...
    if config is not None:
        ...
        # Load the zone-aware camera configuration (FEATURE 4). This is
        # additive: camera_list/policy_data above are untouched, so the
        # existing detection behaviour keeps working even if
        # camera_config.json is missing.
        camera_config_path = opt.camera_config if opt.camera_config else zone_manager.DEFAULT_CAMERA_CONFIG_PATH
        camera_config = zone_manager.load_camera_config(camera_config_path)
        if opt.cpu and not half:
        ...
        for path, img, im0s, vid_cap in dataset:
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                data = policy_data[i]
                # FEATURE 1-3: resolve which zone/camera profile this
                # stream belongs to (dynamic, no hardcoded camera logic).
                cam_key, cam_profile = zone_manager.get_camera_profile(i, camera_config)
                privacy_zone = zone_manager.is_privacy_zone(cam_profile)
                ...
                # FEATURE 3: Locker Room / Privacy Zone cameras only report
                # occupancy -- no PPE/violation detection, no screenshots,
                # no recording, regardless of what the underlying policy
                # detected.
                if privacy_zone:
                    flag = False

                zone_status = "VIOLATION" if flag else "SAFE"
                zone_violation_text = notification_caption if flag else None
                # FEATURE 5: draw the zone/PPE/status overlay on top of the
                # existing detection boxes (does not modify them).
                zone_manager.draw_zone_overlay(im0, cam_key, cam_profile, status=zone_status,
                                                violation_text=zone_violation_text)
                # FEATURE 7 + 8: log every violation with time/camera/zone/severity.
                if flag and not privacy_zone:
                    zone_manager.log_event(cam_key, cam_profile, notification_caption)

                if flag:
                    if privacy_zone:
                        pass  # no screenshots / recording for privacy zones
                    elif time.time() - start_time > flag_for_time: 
                        q.put((im0, save_path, mask_faces, face_detector, api_key, chat_id, notification_caption))
                        timers[i] = time.time()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--camera_config', type=str, help='Load zone-aware camera config from file (default: camera_config.json)')
