...
# --------------------------------------------------------------------
# FEATURE 6: Zone-colored dashboard cards, one per configured camera.
# FEATURE 7: Event log table (time, camera, zone, violation, severity).
# These read camera_config.json + event_log.json, written by
# zone_manager.py / securade.py. They do not touch the existing bar
# chart / image gallery logic above.
# --------------------------------------------------------------------
zone_cards_container = html.Div(id="zone_cards", className="mb-4")
event_log_table = html.Div(id="event_log_table")


def build_zone_cards():
    camera_config = zone_manager.load_camera_config(CAMERA_CONFIG_PATH)
    if not camera_config:
        return html.Div("No camera_config.json found.")
    cards = zone_manager.latest_status_by_camera(camera_config, EVENT_LOG_PATH)
    card_components = []
    for c in cards:
        zi = zone_manager.get_zone_info(c["zone"])
        body = [
            html.H6(c["camera"], className="mb-0"),
            html.Small(c["name"], className="text-muted"),
            html.Div(f"{zi['emoji']} {zi['label']}", className="mt-1"),
            html.Div(c["status"], style={"fontWeight": "bold"}),
        ]
        card_components.append(
            dbc.Col(
                dbc.Card(
                    dbc.CardBody(body),
                    style={"borderLeft": f"6px solid {zi['hex']}"},
                    className="mb-2",
                ),
                width=4,
            )
...
def build_event_log_table():
    events = zone_manager.read_event_log(EVENT_LOG_PATH, limit=100)
    if not events:
        return html.Div("No violations logged yet.")
    df = pd.DataFrame(events)[["time", "date", "camera", "zone", "violation", "severity"]]
    ...
    return dash_table.DataTable(
        ...
        style_data_conditional=[
            ...
            {"if": {"filter_query": '{severity} = "High"'}, "backgroundColor": "#fdf2e3"},
        ],
    )


@callback(
    Output("zone_cards", "children"),
    Output("event_log_table", "children"),
    Input("zone_refresh_interval", "n_intervals"),
)
def refresh_zone_dashboard(n_intervals):
    return build_zone_cards(), build_event_log_table()
...
def serve_layout():
    ...
    [
        header,
        html.H6("Camera Zones", className="mt-2"),
        zone_cards_container,
        html.H6("Violation Event Log"),
        event_log_table,
        dcc.Interval(id="zone_refresh_interval", interval=5000, n_intervals=0),
        dbc.Row(
            [
                dbc.Col(
                    [
